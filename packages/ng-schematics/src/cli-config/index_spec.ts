...
`;
	...
	describe("addAISkills", () => {
		...
		const mockCopilotContent = "# Copilot Instructions\nTest content";
		const mockClaudeContent = "# Claude Instructions\nTest content";

		beforeEach(() => {
			spyOn(globalThis, "fetch").and.callFake((url: string | URL | Request) => {
				const urlStr = url.toString();
				if (urlStr.includes("copilot-instructions.md")) {
					return Promise.resolve(new Response(mockCopilotContent, { status: 200 }));
				}
				if (urlStr.includes("CLAUDE.md")) {
					return Promise.resolve(new Response(mockClaudeContent, { status: 200 }));
				}
				return Promise.resolve(new Response("Not Found", { status: 404 }));
			});
		});

		it("should create AI skill files when addAISkills is true", async () => {
			await runner.runSchematic("cli-config", { addAISkills: true }, tree);
			expect(tree.exists(copilotPath)).toBeTruthy();
			expect(tree.exists(claudePath)).toBeTruthy();
			expect(tree.readContent(copilotPath)).toEqual(mockCopilotContent);
			expect(tree.readContent(claudePath)).toEqual(mockClaudeContent);
		});

		it("should create AI skill files by default (addAISkills not specified)", async () => {
			await runner.runSchematic("cli-config", {}, tree);
			expect(tree.exists(copilotPath)).toBeTruthy();
			expect(tree.exists(claudePath)).toBeTruthy();
		});

		it("should NOT create AI skill files when addAISkills is false", async () => {
			await runner.runSchematic("cli-config", { addAISkills: false }, tree);
			expect(tree.exists(copilotPath)).toBeFalsy();
			expect(tree.exists(claudePath)).toBeFalsy();
		});

		it("should not overwrite existing copilot-instructions.md", async () => {
			const existingContent = "# Existing instructions";
			tree.create(copilotPath, existingContent);

			await runner.runSchematic("cli-config", { addAISkills: true }, tree);
			expect(tree.readContent(copilotPath)).toEqual(existingContent);
			expect(tree.exists(claudePath)).toBeTruthy();
			expect(tree.readContent(claudePath)).toEqual(mockClaudeContent);
		});
		...
		it("should not overwrite existing CLAUDE.md", async () => {
			...
			tree.create(claudePath, existingContent);

			await runner.runSchematic("cli-config", { addAISkills: true }, tree);
			expect(tree.readContent(claudePath)).toEqual(existingContent);
			expect(tree.exists(copilotPath)).toBeTruthy();
			expect(tree.readContent(copilotPath)).toEqual(mockCopilotContent);
		});

		it("should handle fetch failure gracefully", async () => {
			(globalThis.fetch as jasmine.Spy).and.callFake(() =>
			...
			expect(tree.exists(copilotPath)).toBeFalsy();
			expect(tree.exists(claudePath)).toBeFalsy();
			expect(warns).toContain(jasmine.stringMatching(/Could not fetch AI skill files/));
		});
	});
});
