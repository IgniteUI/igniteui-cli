...
function addAISkillsFiles(options: CliConfigOptions): Rule {
	return (tree: Tree, context: SchematicContext) => {
		...
		const igxPackage = resolvePackage(NPM_ANGULAR);
		const skillsSourceDir = normalize(`/node_modules/${igxPackage}/skills`);
		...
		const skillsSource = () => {
			const skillsTree = Tree.empty();
			skillsDir.visit((filePath) => {
				skillsTree.create(relative(skillsSourceDir, filePath), tree.read(filePath)!);
			});
			return skillsTree;
		};
