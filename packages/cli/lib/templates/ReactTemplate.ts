...
export class ReactTemplate implements Template {
    ...
    protected registerSourceFiles(projectPath: string) {
        ...
        if (fs.existsSync(igResPath)) {
            ...
            let dvDep = this.dependencies.filter(x => components.dv.indexOf(x) !== -1).length !== 0;
            let fullDep = this.dependencies.filter(x => components.full.indexOf(x) !== -1).length !== 0;
            let updateImports = dvDep || fullDep;
            ...
            if (!updateImports) {
                ...
                if (dvDep && !igniteuiResFile.includes(dvPath)) {
                    fs.appendFileSync(igResPath, `${'// Ignite UI Charts Required JavaScript File\r\nexport * from "' + dvPath + '";\r\n'}`);
                }
            }
