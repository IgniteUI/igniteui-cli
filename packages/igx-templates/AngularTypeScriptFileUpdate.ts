import { App, FS_TOKEN, IFileSystem, IImport, TemplateDependency, TypeScriptFileUpdate, Util } from "@igniteui/cli-core";
...
export class AngularTypeScriptFileUpdate /* extends TypeScriptFileUpdate */ {
		...
		(rootNode: T) => {
			const conditionalVisitor: ts.Visitor = (node: ts.Node): ts.Node => {
				if (node.kind === ts.SyntaxKind.ArrayLiteralExpression &&
					...
					const alreadyProvided = null; // nodes.map(x => TsNodeUtils.getIdentifierName(x));
	...
	protected initState() {
		this.targetSource = null; // TsUtils.getFileSource(this.targetPath);
	...
	protected addRouteModuleEntry(
		...
		let className: string;
		const fileSource = null; // TsUtils.getFileSource(filePath);
		const relativePath: string = Util.relativePath(this.targetPath, filePath, true, true);
		className = null; // TsNodeUtils.getClassName(fileSource.getChildren());
		...
		<T extends ts.Node>(context: ts.TransformationContext) => (rootNode: T) => {
			const visitNgModule: ts.Visitor = (node: ts.Node): ts.Node => {
				...
				if (node.kind === ts.SyntaxKind.ObjectLiteralExpression &&
					...
					const alreadyImported = null; // props.map(x => TsNodeUtils.getIdentifierName(x));
