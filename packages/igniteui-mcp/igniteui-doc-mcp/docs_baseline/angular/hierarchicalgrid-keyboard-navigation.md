...
```
In order to add custom keyboard navigation to igxHierarchicalGrid child grids too, each child grid should subscribe to [`gridKeydown`](mcp:get_api_reference?platform=angular&component=IgxHierarchicalGridComponent&member=gridKeydown) event. That's why in example above we have registered and event handler for for the [`gridCreated`](mcp:get_api_reference?platform=angular&component=IgxRowIslandComponent&member=gridCreated) event:
```typescript
