...
```
Then define a Grid with bound data source and [`rowEditable`](mcp:get_api_reference?platform=angular&component=IgxGridComponent&member=rowEditable) set to true and an [Action Strip](../action-strip.md) component with editing actions enabled. The [`addRow`](mcp:get_api_reference?platform=angular&component=IgxGridEditingActionsComponent&member=addRow) input controls the visibility of the button that spawns the row adding UI.
```html
...
> [!NOTE]
> The IgxGridEditingActions input controlling the visibility of the add row button may use the action strip context (which is of type [`RowType`](mcp:get_api_reference?platform=angular&component=RowType)) to fine tune which records the button shows for.
The internal [`IgxBaseTransactionService`](mcp:get_api_reference?platform=angular&component=IgxBaseTransactionService) is automatically provided for Grid. It holds pending cell changes until the row state is submitted or cancelled.
