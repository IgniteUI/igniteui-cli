...
Great, and now let's prepare for the search API of our Grid! We can create a few properties, which can be used for storing the currently searched text and whether the search is case sensitive and/or by an exact match.
```typescript
...
- The `grid.lastSearchInfo.matchCount` value will give us the total results count.
...
<div class="resultsText">
    <span *ngIf="grid.lastSearchInfo.matchCount > 0">
        {{ grid.lastSearchInfo.activeMatchIndex + 1 }} of {{ grid.lastSearchInfo.matchCount }} results
    </span>
    <span *ngIf="grid.lastSearchInfo.matchCount == 0">
        No results
    </span>
