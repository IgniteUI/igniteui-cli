...
The Ignite UI for React [`IgrGrid`](mcp:get_api_reference?platform=react&component=IgrGrid) can read Excel data that is copied to the clipboard. In this section we will show you how to do this with some custom code.

## React Paste from Excel Example

This sample demonstrates how to implement pasting from Excel into the [`IgrGrid`](mcp:get_api_reference?platform=react&component=IgrGrid) Material UI table.
