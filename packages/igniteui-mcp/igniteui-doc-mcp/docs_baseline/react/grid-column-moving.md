...
> [!Note]
> If the pinned area exceeds its maximum allowed width (80% of the total [`IgrGrid`](mcp:get_api_reference?platform=react&component=IgrGrid) width), a visual clue notifies the end user that the drop operation is forbidden and pinning is not possible. This means you won't be allowed to drop a column in the pinned area.
...
**Column moving** feature is enabled on a per-grid level, meaning that the [`IgrGrid`](mcp:get_api_reference?platform=react&component=IgrGrid) could have either movable or immovable columns. This is done via the [`moving`](mcp:get_api_reference?platform=react&component=IgrGrid&member=moving) input of the [`IgrGrid`](mcp:get_api_reference?platform=react&component=IgrGrid).
...
[`move`](mcp:get_api_reference?platform=react&component=IgrColumn&member=move) - Moves a column to a specified visible index. If the passed index parameter is invalid (is negative, or exceeds the number of columns), or if the column is not allowed to move to this index (if inside another group), no operation is performed.
...
You can subscribe to the `ColumnMovingEnd` event of the [`IgrGrid`](mcp:get_api_reference?platform=react&component=IgrGrid) to implement some custom logic when a column is dropped to a new position. For example, you can cancel dropping the **Category** column after the **Change On Year(%)** column in the following code snippet.
