---
title: React Licensing | Dock Manager | Ignite UI for React | Infragistics
_description: Learn how to use Infragistics' Ignite UI for React licensed npm feed.
_keywords: npm package license, Ignite UI for React license feed,  licensing
mentionedTypes: []
_tocName: Licensing
---
...
Ignite UI for React comprises packages available under either an MIT or a commercial license. This licensing model supports both commercial and permissive open-source usage, depending on the specific components, modules, and services you incorporate into your project.
...
Have in mind that we have set the Ignite UI for React package to be scoped, meaning that no changing the registries is needed if you want to install packages from our private feed and from npmjs.org simultaneously.
...
- Do a global replace for all trial packages and change then to the licensed scoped version across all files in your project. For example, replace all `igniteui-react-dockmanager` references to `@infragistics/igniteui-react-dockmanager` The change should affect all imports where such packages are used.
...
So, if you've already adopted npm and you have an Ignite UI for React license, don't hesitate setting up the Infragistics private feed and boost your productivity, using the full potential of Ignite UI for React.

## Access Token Usage

You can also authenticate to our private npm feed using an access token, which you can acquire through your [infragistics.com user account](https://account.infragistics.com/access-tokens). The access token authentication is the preferred alternative when you want to integrate a CI process in a publicly accessible repository, which uses the Ignite UI for React licensed packages.
