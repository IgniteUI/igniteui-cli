...
As you can see in the demo above, the [`IgrTreeGrid`](mcp:get_api_reference?platform=react&component=IgrTreeGrid) provides three size options:  **small**, **medium** and **large**. The code snippet below shows how to set `--ig-size` either inline or part of a CSS class:
