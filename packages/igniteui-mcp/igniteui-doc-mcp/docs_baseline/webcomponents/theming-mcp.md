---
title: Web Components Theming MCP | Infragistics 
_description: Web Components works with the Ignite UI Theming MCP server that allows you to create custom themes, palettes, typography, and elevations for your Ignite UI for Web Components applications. Learn how to use the MCP server to generate and apply custom themes that match your brand and design requirements.
_keywords: Ignite UI for Web Components controls, Web Components widgets, web widgets, UI widgets, Components Suite, Artificial Intelligence, AI, MCP, Model Context Protocol, Theming, Custom Themes, Palettes, Typography, Elevations
_license: MIT
mentionedTypes: []
...
The server supports all four Ignite UI design systems — **Material**, **Bootstrap**, **Fluent**, and **Indigo** — in both light and dark variants. While this guide focuses on Web Components, the MCP server also works with all Ignite UI component libraries from Infragistics. The `detect_platform` tool reads your `package.json` and selects the correct import paths and selectors automatically.
...
> _"Create a complete Material Design dark theme for my Web Components app with primary #2563eb and coral secondary #f97316."_
...
```markdown
---
description: Ignite UI for Web Components theming conventions
globs: ["**/*.scss", "**/styles/**"]
---
...
> _"I'm starting a new Web Components project with Ignite UI. Create a complete Material Design light theme with primary #2563eb, secondary #f97316, and Roboto font."_
...
> _"Style the flat button with a purple background #8b5cf6 and white text, with a lighter purple #a78bfa on hover."_
...
If `detect_platform` returns `null` or `generic`, make sure your `package.json` lists an Ignite UI package (e.g., `igniteui-webcomponents`) as a dependency. You can also tell the AI explicitly: _"Use the Web Components platform."_
