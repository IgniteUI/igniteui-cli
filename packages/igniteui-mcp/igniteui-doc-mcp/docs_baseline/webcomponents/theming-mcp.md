...
_description: Use the Ignite UI Theming MCP server to generate Ignite UI for Web Components palettes, themes, typography, and design token overrides through your AI assistant.
...
If `detect_platform` returns `null` or `generic`, make sure your `package.json` lists an Ignite UI package (e.g., `igniteui-webcomponents`) as a dependency.

You can also tell the AI explicitly: _"Use the Web Components platform."_
