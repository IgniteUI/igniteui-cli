---
title: Web Components Grid Sizing - Ignite UI for Web Components
_description: Understand how the Web Components Grid sizing works and learn how to use the width and height in order to accommodate the different scenarios that users can have.
_keywords: Sizing , Web Components, Grid, IgcGrid, Ignite UI for Web Components, Infragistics
_license: commercial
...
# Web Components Grid Sizing

The Ignite UI for Web Components Sizing feature in Web Components Grid is an important aspect of creating responsive and user-friendly grid interfaces. The Web Components Grid Sizing feature enables users to adjust the [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=width) and [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=height) of the grid to accommodate different screen sizes, content, or user preferences. In terms of border and padding size for the [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent), they are taken into its width/height size calculations or also known as [Border box](https://developer.mozilla.org/en-US/docs/Web/CSS/box-sizing) sizing. It is applied in all scenarios.

> [!Note]
> If the Border box sizing is overridden by the user we cannot guarantee that the [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent) will size correctly.

## Web Components Grid Width

If the [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=width) input does not have value assigned, its default value is `100%` and the [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent) tries to fill the available space. You can check how the grid reacts to it in the `Percentages` section.

> [!Note]
> Setting manually the [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=width) style of the [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent) itself will result in an unexpected behavior.

### Null

The grid's [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=width) can accepts value of `null`, which when set, renders all columns in the DOM. The grid sizes accordingly so there is no grid horizontal scrollbar since column virtualization is not applied.
...
When the [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=width) of the [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent) is set to percentages it will size the grid according to the parent element's width. If the parent element does not have width specified the [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent) will size relative to the browser window.

- For example, if we set the grid [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=width) input to `100%` and there is no parent element it will fill 100% of the available width of the browser window. If it is resized the grid will resize as well accordingly.
...
## Web Components Grid Height

By default if no height is defined for the [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent), it will be set to `100%`. You can check how the grid reacts depending on the DOM structure in the `Percentages` section.

> [!Note]
> Setting manually the [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=height) style of the [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent) itself will result in an unexpected behavior.

### Null

The [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent) [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=height) input can accept `null` value, which when set, displays all rows with no scrollbar no matter how many they are. In this case, there is no vertical virtualization since the grid renders all rows anyway.
...
- If there is a parent element with defined [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=height), the grid will still render all rows and not be affected. Let's say the parent has [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=height) of `650px`. If it has `overflow` set to `auto` or `scroll`, it will render a vertical scrollbar but the grid will still be unaffected:
...
Setting the [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent) [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=height) in pixels is more straightforward since the grid will size to that specific size in all occasions similarly to how [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=width) is set in pixels.
...
- If the number of rows exceeds the visible area of the grid when [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=height) is set to pixels a vertical scrollbar will be rendered. For example, a grid with `500px` height set and 14 rows will be rendered the following way:

<img src="../../../images/grid_sizing/height-500px-14rows-v2.jpg" alt="height-500px-14rows-v2" style="width: 80%"/>

- If there is a parent element with [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=height) defined, unless it has `overflow` set to `auto` or `scroll`, the grid will still be fully visible. Otherwise it will render a scrollbar.
...
When the [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=height) input is set to percentages, the [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent) will size based on the parent element height. If the parent element has its [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=height) set in pixels or percentages, the grid will size relative to the size of the parent.
...
Let's have [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=width) set to `1200px` and the parent element not having any size applied to it:
...
- If we set the parent element height to `800px` and the [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent) to `100%` height this means that the grid will be sized to 100 percentages of `800px`.

<img src="../../../images/grid_sizing/height-percent-100-parent-800px-v2.jpg" alt="height-percent-100-parent-800px-v2" style="width: 80%"/>

- If the [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent) [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=height) is set to a number bigger than `100%` and the parent element has height, for the parent to render scrollbar it again needs to have `overflow` set to `auto` or `scroll`. Otherwise the grid will be fully visibly and size relative to the parent size.
...
## Web Components Grid Column Sizing
