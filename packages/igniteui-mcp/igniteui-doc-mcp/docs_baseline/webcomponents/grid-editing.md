...
The Ignite UI for Web Components Cell Editing feature in Web Components Grid provides an easy way to perform data manipulation operations like creating, updating, and deleting records. The [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent) provides you with a powerful public API which allows you to customize the way these operations are performed. The data manipulation phases are:
...
> Keep in mind that if the column is not editable, you can still modify its value through the public API exposed by the [`IgcGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent).

The [`rowEditable`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=rowEditable) property enables you to specify the following options:
...
- `CellEditEnter` - Prevents entering cell edit. If [`rowEditable`](mcp:get_api_reference?platform=webcomponents&component=IgcGridComponent&member=rowEditable) is enabled, row edit will be triggered, although cell edit will remain forbidden.
