...
_description: Understand how the Web Components Tree Grid sizing works and learn how to use the width and height in order to accommodate the different scenarios that users can have.
_keywords: Sizing , Web Components, Tree Grid, IgcTreeGrid, Ignite UI for Web Components, Infragistics
_license: commercial
...
The Ignite UI for Web Components Sizing feature in Web Components Tree Grid is an important aspect of creating responsive and user-friendly grid interfaces. The Web Components Tree Grid Sizing feature enables users to adjust the [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=width) and [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=height) of the grid to accommodate different screen sizes, content, or user preferences. In terms of border and padding size for the [`IgcTreeGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent), they are taken into its width/height size calculations or also known as [Border box](https://developer.mozilla.org/en-US/docs/Web/CSS/box-sizing) sizing. It is applied in all scenarios.

> [!Note]
> If the Border box sizing is overridden by the user we cannot guarantee that the [`IgcTreeGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent) will size correctly.

## Web Components Tree Grid Width

If the [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=width) input does not have value assigned, its default value is `100%` and the [`IgcTreeGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent) tries to fill the available space. You can check how the grid reacts to it in the `Percentages` section.

> [!Note]
> Setting manually the [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=width) style of the [`IgcTreeGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent) itself will result in an unexpected behavior.

### Null

The grid's [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=width) can accepts value of `null`, which when set, renders all columns in the DOM. The grid sizes accordingly so there is no grid horizontal scrollbar since column virtualization is not applied.
...
When the [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=width) of the [`IgcTreeGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent) is set to percentages it will size the grid according to the parent element's width. If the parent element does not have width specified the [`IgcTreeGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent) will size relative to the browser window.

- For example, if we set the grid [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=width) input to `100%` and there is no parent element it will fill 100% of the available width of the browser window. If it is resized the grid will resize as well accordingly.
...
By default if no height is defined for the [`IgcTreeGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent), it will be set to `100%`. You can check how the grid reacts depending on the DOM structure in the `Percentages` section.

> [!Note]
> Setting manually the [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=height) style of the [`IgcTreeGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent) itself will result in an unexpected behavior.

### Null

The [`IgcTreeGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent) [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=height) input can accept `null` value, which when set, displays all rows with no scrollbar no matter how many they are. In this case, there is no vertical virtualization since the grid renders all rows anyway.
...
Setting the [`IgcTreeGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent) [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=height) in pixels is more straightforward since the grid will size to that specific size in all occasions similarly to how [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=width) is set in pixels.
...
- If there is a parent element with [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=height) defined, unless it has `overflow` set to `auto` or `scroll`, the grid will still be fully visible. Otherwise it will render a scrollbar.
...
When the [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=height) input is set to percentages, the [`IgcTreeGridComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent) will size based on the parent element height. If the parent element has its [`height`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=height) set in pixels or percentages, the grid will size relative to the size of the parent.
...
Let's have [`width`](mcp:get_api_reference?platform=webcomponents&component=IgcTreeGridComponent&member=width) set to `1200px` and the parent element not having any size applied to it:
