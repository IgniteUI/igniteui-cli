...
The [`IgcTileManagerComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTileManagerComponent) provides a base tile layout behavior, managing the placement of tiles in maximized or normal state. The tiles can be sized independently of each other and used to form complex layouts. End users can reorder tiles by dragging and dropping them, providing a flexible and intuitive experience.

The Tile Manager offers two components that we can use:

- [`IgcTileComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTileComponent) - This component represents an individual tile displayed within the Tile Manager.
- [`IgcTileManagerComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTileManagerComponent) - This is the main component that contains all of the tile components, serving as the container for the entire tile layout.
...
Another property that can be used in the Tile Manager is the [`gap`](mcp:get_api_reference?platform=webcomponents&component=IgcTileManagerComponent&member=gap) property, which defines the space between tiles. The value of the [`gap`](mcp:get_api_reference?platform=webcomponents&component=IgcTileManagerComponent&member=gap) property must be a number followed by a length unit (e.g., px, rem, em, ...). This value will apply to both the horizontal gap (width) and the vertical gap (height) between tiles.
...
We also have properties for setting the minimum width of the columns ([`minColumnWidth`](mcp:get_api_reference?platform=webcomponents&component=IgcTileManagerComponent&member=minColumnWidth)) and the minimum height of the rows ([`minRowHeight`](mcp:get_api_reference?platform=webcomponents&component=IgcTileManagerComponent&member=minRowHeight)) in the Tile Manager. Similar to the gap property, the values for these properties must be a number followed by a length unit. These values will define the minimum width for all columns and the minimum height for all rows in the Tile Manager.
...
- The [`colSpan`](mcp:get_api_reference?platform=webcomponents&component=IgcTileComponent&member=colSpan) property specifies how many columns the tile will span across in the layout, allowing you to control its horizontal size.
- The [`rowSpan`](mcp:get_api_reference?platform=webcomponents&component=IgcTileComponent&member=rowSpan) property determines how many rows the tile will span vertically, adjusting the tile's height within the layout.
- The [`colStart`](mcp:get_api_reference?platform=webcomponents&component=IgcTileComponent&member=colStart) property specifies the starting column where the tile is placed.
- The [`rowStart`](mcp:get_api_reference?platform=webcomponents&component=IgcTileComponent&member=rowStart) property specifies the starting row where the tile is placed.
- The [`disableFullscreen`](mcp:get_api_reference?platform=webcomponents&component=IgcTileComponent&member=disableFullscreen) property hides the default fullscreen action button.
- The [`disableMaximize`](mcp:get_api_reference?platform=webcomponents&component=IgcTileComponent&member=disableMaximize) property hides the default maximize toggle action button.
- The [`disableResize`](mcp:get_api_reference?platform=webcomponents&component=IgcTileComponent&member=disableResize) property prevents the tile from being resized by the user.
...
If you want to display just one of the two buttons, you can set either [`disableMaximize`](mcp:get_api_reference?platform=webcomponents&component=IgcTileComponent&member=disableMaximize) or [`disableFullscreen`](mcp:get_api_reference?platform=webcomponents&component=IgcTileComponent&member=disableFullscreen) property. To customize the appearance you can use the `maximize-action` slot for the maximize button, or the `fullscreen-action` slot for the fullscreen button.
...
- A tile cannot be resized smaller than its defined minimum width or height ([`minColumnWidth`](mcp:get_api_reference?platform=webcomponents&component=IgcTileManagerComponent&member=minColumnWidth), [`minRowHeight`](mcp:get_api_reference?platform=webcomponents&component=IgcTileManagerComponent&member=minRowHeight)).
- A tile cannot exceed the maximum available horizontal space in the grid.

## Reorder

You can reorder tiles in the Tile Manager using the drag-and-drop feature. By default, tiles are not draggable. To enable this functionality, set the [`dragMode`](mcp:get_api_reference?platform=webcomponents&component=IgcTileManagerComponent&member=dragMode) property on the Tile Manager to either `tile` or `tile-header`.
...
- The [`saveLayout`](mcp:get_api_reference?platform=webcomponents&component=IgcTileManagerComponent&member=saveLayout) method allows you to save the current arrangement of tiles in the Tile Manager, it captures the current order, size and position of all tiles, so you can later restore it to this exact configuration.
- The [`loadLayout`](mcp:get_api_reference?platform=webcomponents&component=IgcTileManagerComponent&member=loadLayout) method enables you to load a previously saved layout. When called, it restores the tiles to the exact state they were in when the layout was saved, including their order, size and position.
...
You can also customize the appearance of the two components - [`IgcTileManagerComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTileManagerComponent) and [`IgcTileComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTileComponent).
The [`IgcTileManagerComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTileManagerComponent) exposes only one CSS property - `base` which can be used for styling the base wrapper of the Tile Manager.
The [`IgcTileComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTileComponent) exposes several CSS properties we can use:
...
- [`IgcTileManagerComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTileManagerComponent)
- [`IgcTileComponent`](mcp:get_api_reference?platform=webcomponents&component=IgcTileComponent)
