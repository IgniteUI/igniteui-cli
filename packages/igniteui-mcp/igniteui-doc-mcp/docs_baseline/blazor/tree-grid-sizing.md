...
The Ignite UI for Blazor Sizing feature in Blazor Tree Grid is an important aspect of creating responsive and user-friendly grid interfaces. The Blazor Tree Grid Sizing feature enables users to adjust the [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) and `Height` of the grid to accommodate different screen sizes, content, or user preferences. In terms of border and padding size for the [`IgbTreeGrid`](mcp:get_api_reference?platform=blazor&component=IgbTreeGrid), they are taken into its width/height size calculations or also known as [Border box](https://developer.mozilla.org/en-US/docs/Web/CSS/box-sizing) sizing. It is applied in all scenarios.

> [!Note]
> If the Border box sizing is overridden by the user we cannot guarantee that the [`IgbTreeGrid`](mcp:get_api_reference?platform=blazor&component=IgbTreeGrid) will size correctly.

## Blazor Tree Grid Width

If the [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) input does not have value assigned, its default value is `100%` and the [`IgbTreeGrid`](mcp:get_api_reference?platform=blazor&component=IgbTreeGrid) tries to fill the available space. You can check how the grid reacts to it in the `Percentages` section.

> [!Note]
> Setting manually the [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) style of the [`IgbTreeGrid`](mcp:get_api_reference?platform=blazor&component=IgbTreeGrid) itself will result in an unexpected behavior.
...
When the [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) of the [`IgbTreeGrid`](mcp:get_api_reference?platform=blazor&component=IgbTreeGrid) is set to percentages it will size the grid according to the parent element's width. If the parent element does not have width specified the [`IgbTreeGrid`](mcp:get_api_reference?platform=blazor&component=IgbTreeGrid) will size relative to the browser window.
...
By default if no height is defined for the [`IgbTreeGrid`](mcp:get_api_reference?platform=blazor&component=IgbTreeGrid), it will be set to `100%`. You can check how the grid reacts depending on the DOM structure in the `Percentages` section.

> [!Note]
> Setting manually the `Height` style of the [`IgbTreeGrid`](mcp:get_api_reference?platform=blazor&component=IgbTreeGrid) itself will result in an unexpected behavior.

### Null

The [`IgbTreeGrid`](mcp:get_api_reference?platform=blazor&component=IgbTreeGrid) `Height` input can accept `null` value, which when set, displays all rows with no scrollbar no matter how many they are. In this case, there is no vertical virtualization since the grid renders all rows anyway.
...
Setting the [`IgbTreeGrid`](mcp:get_api_reference?platform=blazor&component=IgbTreeGrid) `Height` in pixels is more straightforward since the grid will size to that specific size in all occasions similarly to how [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) is set in pixels.
...
When the `Height` input is set to percentages, the [`IgbTreeGrid`](mcp:get_api_reference?platform=blazor&component=IgbTreeGrid) will size based on the parent element height. If the parent element has its `Height` set in pixels or percentages, the grid will size relative to the size of the parent.
...
- If we set the parent element height to `800px` and the [`IgbTreeGrid`](mcp:get_api_reference?platform=blazor&component=IgbTreeGrid) to `100%` height this means that the grid will be sized to 100 percentages of `800px`.

<img src="../../../images/grid_sizing/height-percent-100-parent-800px-v2.jpg" alt="height-percent-100-parent-800px-v2" style="width: 80%"/>

- If the [`IgbTreeGrid`](mcp:get_api_reference?platform=blazor&component=IgbTreeGrid) `Height` is set to a number bigger than `100%` and the parent element has height, for the parent to render scrollbar it again needs to have `overflow` set to `auto` or `scroll`. Otherwise the grid will be fully visibly and size relative to the parent size.
