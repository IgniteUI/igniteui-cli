...
The Ignite UI for Blazor Sizing feature in Blazor Hierarchical Grid is an important aspect of creating responsive and user-friendly grid interfaces. The Blazor Hierarchical Grid Sizing feature enables users to adjust the [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) and `Height` of the grid to accommodate different screen sizes, content, or user preferences. In terms of border and padding size for the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid), they are taken into its width/height size calculations or also known as [Border box](https://developer.mozilla.org/en-US/docs/Web/CSS/box-sizing) sizing. It is applied in all scenarios.

> [!Note]
> If the Border box sizing is overridden by the user we cannot guarantee that the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) will size correctly.

## Blazor Hierarchical Grid Width

If the [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) input does not have value assigned, its default value is `100%` and the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) tries to fill the available space. You can check how the grid reacts to it in the `Percentages` section.

> [!Note]
> Setting manually the [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) style of the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) itself will result in an unexpected behavior.

### Null

The grid's [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) can accepts value of `null`, which when set, renders all columns in the DOM. The grid sizes accordingly so there is no grid horizontal scrollbar since column virtualization is not applied.

- If there are 6 columns and none of them has width defined, the grid will have [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) of `816px`, because each column by default have assigned [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) of `136px` in this scenario. Same will happen if the columns have [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) in percentages. If vertical scrollbar is rendered or there are features that render additional columns their width will be added also.
...
When the [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) of the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) is set to percentages it will size the grid according to the parent element's width. If the parent element does not have width specified the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) will size relative to the browser window.

- For example, if we set the grid [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) input to `100%` and there is no parent element it will fill 100% of the available width of the browser window. If it is resized the grid will resize as well accordingly.
...
- If we have a parent element with [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) of `1000px` and have the grid's [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) set to `150%`, the calculated grid width will be `1500px`. In this case the grid will still render fully visible but if we set `overflow: auto` of the parent, that parent will render scrollbar on its own.
...
By default if no height is defined for the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid), it will be set to `100%`. You can check how the grid reacts depending on the DOM structure in the `Percentages` section.

> [!Note]
> Setting manually the `Height` style of the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) itself will result in an unexpected behavior.

### Null

The [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) `Height` input can accept `null` value, which when set, displays all rows with no scrollbar no matter how many they are. In this case, there is no vertical virtualization since the grid renders all rows anyway.
...
Setting the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) `Height` in pixels is more straightforward since the grid will size to that specific size in all occasions similarly to how [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) is set in pixels.
...
When the `Height` input is set to percentages, the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) will size based on the parent element height. If the parent element has its `Height` set in pixels or percentages, the grid will size relative to the size of the parent.
...
Let's have [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) set to `1200px` and the parent element not having any size applied to it:
...
- If a column does not have specified [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) and the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) has [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) set to `null`, it will be sized to the minimum of `136px`. This means that for a grid with [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) `null` and 6 columns that don't have width, each column will be sized to `136px`.
...
- Same applies if multiple columns does not have [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) specified, all will divide the available space between each other equally. In the next illustration the first column has [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) set to `100px`.
...
When columns have set specific [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) in pixels, they stick to that size, unless they are resized manually. Since the combined [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) of the columns is static, it can be less than the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) or exceed it.

- If the combined `width` of all columns is less than the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width), there would be an empty are inside the grid that the columns wouldn't be able to fill. This is the expected behavior of the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid). In the next example the columns have `150px` width.

<img src="../../../images/grid_sizing/columns-150px-empty-area-v2.jpg" alt="columns-150px-empty-area-v2" style="width: 80%"/>

- If the combined [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) of all columns is bigger than the actual [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width), a horizontal scrollbar will be rendered. In the next example each of the 6 columns have width of `300px` and grid has width of `1200px`, which means that the columns combined have excess of `600px` that goes out of bounds.

<img src="../../../images/grid_sizing/columns-150px-extra-area-v2.jpg" alt="columns-150px-extra-area-v2" style="width: 80%"/>

### Percentages

When columns have set [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) in percentages, their size is calculated relatively to the grid size. It is similar to how width in pixels works, but provides also responsiveness to the columns which means that when the grid is resized, the columns also will resize accordingly.
...
- If columns are set in percentages and the grid [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) is set to `null`, it would apply [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) of `136px` to each column. That is because the columns cannot be sized relatively to the grid, since it doesn't have [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) itself and relies on its content to be sized when its [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) is `null`. In the following example all 6 columns have [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) set to `50%`:
...
Because the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) usually contains children, they can also have their [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) and `Height` specified, in order to accommodate different scenarios. Since the children are defined using [`IgbRowIsland`](mcp:get_api_reference?platform=blazor&component=IgbRowIsland) template, this means that all children in the same level and island will have the same [`Width`](mcp:get_api_reference?platform=blazor&component=IgbRowIsland&member=Width) and [`Height`](mcp:get_api_reference?platform=blazor&component=IgbRowIsland&member=Height) property applied to them.

### Width

The [`Width`](mcp:get_api_reference?platform=blazor&component=IgbRowIsland&member=Width) for the children does not behave much different than the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) itself, since each child grid is instance of [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) as well.

The only difference is that the user cannot change the parent element of the child grid. That's why when the [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width) is set to percentages, the `100%` width allocated for the child is smaller than the parent grid [`Width`](mcp:get_api_reference?platform=blazor&component=IgbColumn&member=Width). This is so that it is easier to distinguish when it is expanded. The following image displays the default child grid sizes, since it defaults to `100%` width.

<img src="../../../images/grid_sizing/hgrid-width-percentages-v2.jpg" alt="hgrid-width-percentages-v2" style="width: 80%"/>

### Height

The `Height` of each child in the [`IgbHierarchicalGrid`](mcp:get_api_reference?platform=blazor&component=IgbHierarchicalGrid) behaves also similarly to the `Height` of the root level grid.
