...
The following sample depicts a [`IgbFinancialChart`](mcp:get_api_reference?platform=blazor&component=IgbFinancialChart) showing the stock trend of Microsoft between 2013 and 2017 with a **QuinticFit** trendline initially applied. There is a drop-down that will allow you to change the type of trendline that is applied, and all possible trendline types are listed within that drop-down.
