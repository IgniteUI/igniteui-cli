...
The [`IgbIcon`](mcp:get_api_reference?platform=blazor&component=IgbIcon) doesn't contain any icons on its own. It's a conduit for displaying any _registered_ SVG images.
...
The icon component can be styled by applying styles directly to the [`IgbIcon`](mcp:get_api_reference?platform=blazor&component=IgbIcon) element;
