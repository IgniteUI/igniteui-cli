...
You can share and add multiple axes in the same plot area of the Blazor [`IgbDataChart`](mcp:get_api_reference?platform=blazor&component=IgbDataChart). It a common scenario to use share [`IgbTimeXAxis`](mcp:get_api_reference?platform=blazor&component=IgbTimeXAxis) and add multiple [`IgbNumericYAxis`](mcp:get_api_reference?platform=blazor&component=IgbNumericYAxis) to plot many data sources that have wide range of values (e.g. stock prices and stock trade volumes).
