...
const INPUT_DIR = join(ROOT, "dist", "docs_prepeared", "react");
const OUTPUT_DIR = join(ROOT, "dist", "docs_final", "react");
...
function getSystemPrompt(): string {
  return `You are compressing Infragistics Ignite UI for React component documentation. Your goal is to reduce file size by ~50% while keeping the doc useful for developers building with these components.
...
- **component**: The Ignite UI for React component class name(s) documented here (e.g. IgrGrid, IgrButton, IgrCombo, IgrDatePicker). Use the PascalCase Igr-prefixed name as used in React. React components do NOT use Angular-style suffixes like Component or Directive — just use the base name (e.g. IgrGrid, not IgrGridComponent). Comma-separate multiple names. These rules are strict:
  - Every name MUST begin with \`Igr\`. A class or function in the sample code that does not start with \`Igr\` is application code, not a library component.
  - NEVER list things the sample application defines for itself — \`App\`, \`MyComponent\`, \`*Sample\`, demo hooks, demo helpers, or local state types. List only components from the Ignite UI library, even when the sample's own code is more prominent.
...
3. **Boilerplate in code blocks**:
   ...
   - Keep JSX/TSX component usage examples intact — these are the primary code format in React docs
...
- **Never change code patterns or structure.** If the original uses a lazy getter (\`private _foo = null; public get foo() { if(!this._foo) this._foo = new Foo(); return this._foo; }\`), do not replace it with direct field initialization (\`private foo = new Foo()\`). If the original wraps text in \`<span id="x">\`, keep the \`<span>\`. Compress by removing entire methods/blocks, not by rewriting the ones you keep.
- **Always mark truncation with a comment.** Whenever you remove items from an array, list, JSX children, or repeated block, insert a \`// ...\` or \`{/* ... */}\` comment at the removal point. Examples: \`[1, 2, 3, 4].map(...)\` shortened to \`[1, 2].map(...)\` is WRONG — use \`[1, 2, /* ... */].map(...)\` instead. Dropping a child component from JSX siblings requires a \`{/* ... more items */}\` comment. Dropping an \`IgrLinearGraphRange\` from a list of 3 requires \`{/* ... additional ranges */}\`.
- **Code blocks must be self-consistent.** If a code block references a variable, function, data model field, or prop, that reference must either be defined within the same block or in an adjacent block. Do not remove a definition (like \`const mods = [...]\`) while keeping code that uses it (\`mods.forEach(...)\`). Do not remove fields from a data model class while keeping JSX that binds to those fields.
- **Do not strip component props from JSX.** If the original has \`<IgrDataToolTipLayer name="tooltips" />\`, keep the \`name\` prop. Only remove props that are purely cosmetic (inline style for demo layout like \`style="height: 600px"\`).
- **Use correct comment syntax.** Inside JSX/TSX markup use \`{/* comment */}\`, inside TypeScript code use \`// comment\`. Never use HTML comments (\`<!-- -->\`) inside JSX — they are not valid and cause runtime errors.
...
- Do NOT synthesize simplified code snippets to replace originals. If the original shows a class component, do not rewrite it as a function component. If the original passes a prop imperatively via a ref callback, do not convert it to a JSX prop with a made-up value. Compress by trimming the original code, not by rewriting it.`;
}
...
async function batchSubmit(opts: CliArgs): Promise<void> {
  ...
  console.log(`\nNext step: npm run compress:react -- --batch poll`);
}
...
async function processBatchResults(
  ...
  if (failedFiles.length > 0 || invalidFiles.length > 0) {
    console.log(`\nTo retry failed/invalid files: npm run compress:react -- --batch retry`);
  }
}
...
async function main() {
  ...
  if (!existsSync(INPUT_DIR)) {
    console.error(`Error: Input directory not found: ${INPUT_DIR}`);
    console.error("Run 'npm run inject:react' first.");
    process.exit(1);
  }
