...
const INPUT_DIR = join(ROOT, "dist", "docs_prepeared", "angular");
const OUTPUT_DIR = join(ROOT, "dist", "docs_final", "angular");
...
function parseArgs(): CliArgs {
  ...
  const opts: CliArgs = {
    model: process.env.COMPRESS_MODEL || "gpt-5.6-luna",
    minSize: 0,
    dryRun: false,
    delay: 0.5,
  };
...
function getSystemPrompt(): string {
  return `You are compressing Infragistics Ignite UI for Angular component documentation. Your goal is to reduce file size by ~50% while keeping the doc useful for developers building with these components.
...
2. **Redundant code examples** — when multiple examples show minor variations of the same pattern, keep the most complete one and summarize what the others demonstrated in one line. However, if two sections cover **different component types or sub-types** (e.g. Stacked Area Chart vs Stacked Spline Area Chart, or Category Chart vs Data Chart), they are NOT redundant — each must keep its own code example showing the distinct component/series type, even if the surrounding boilerplate is similar. The key differentiator is the component tag or series type, not the data or layout code around it.

3. **Boilerplate in code blocks**:
   - Remove repeated module/import setup (\`@NgModule\`, \`IgxModule\` imports) if already shown earlier in the same doc
   - Remove or minimize SCSS/CSS blocks that only set margins, widths, or trivial styling
   - Do NOT merge separate code blocks (TypeScript, HTML, SCSS) into a single inline component. If the original has separate \`templateUrl\`/\`styleUrls\` files shown as individual code blocks, keep them as separate code blocks. Never inline HTML into a \`template\` string or CSS into \`styles\` array.
...
- **Never rewrite or "fix" code from the original.** Preserve property names, values, and casing exactly as they appear in the source, even if they look like typos. Do not rename variables, change method signatures, or "correct" apparent mistakes.
- **Never change literal values.** If the original sets a property to a specific value, use that exact value. Do not substitute different numbers, strings, or enum values.
- **Never change code patterns or structure.** If the original uses a getter, service injection, or Observable pattern, keep that pattern — do not replace it with a simpler alternative. If the original wraps content in specific HTML elements, keep those elements. Compress by removing entire methods/blocks, not by rewriting the ones you keep.
- **Always mark truncation with a comment.** Whenever you remove items from an array, list, template siblings, or repeated block, insert a \`// ...\` or \`<!-- ... -->\` comment at the removal point. Dropping an \`igx-column\` from a list of 5 requires a \`<!-- ... additional columns -->\` comment. Shortening \`[1, 2, 3, 4]\` to \`[1, 2]\` requires a \`/* ... */\` comment.
- **Code blocks must be self-consistent.** If a code block references a variable, function, data model field, or selector, that reference must either be defined within the same block or in an adjacent block. Do not remove a definition (like an \`@NgModule\` imports array) while keeping code that depends on it. Do not remove properties from a data model/interface while keeping template bindings to those properties.
- **Do not strip component properties or directives from templates.** If the original has \`<igx-grid [data]="localData" [autoGenerate]="false">\`, keep all bindings. Only remove properties that are purely cosmetic layout (inline style for demo dimensions like \`style="height: 600px"\`).
...
3. **At least one code example per section that had one in the original** — must include the component usage, not just the data. This applies to every section including variant/sub-type sections (e.g. Step Area Chart, Stacked Area Chart, Radial Area Chart must each keep their own example). If the original section had a code example, the compressed version must keep at least the key template/component snippet — even a 3-5 line snippet is fine. Do NOT replace a code example with only a text description like "follows the same pattern" or "see external examples". If a section has a complex/advanced example (e.g. showing multiple directives, event handling, template references), trim it by reducing repetitive siblings or data, but preserve the structural pattern. **Do NOT merge separate sections into a combined section** — if the original has separate ## or ### sections for different variants, keep them as separate sections in the output.
...
- Do NOT synthesize simplified code snippets to replace originals. If the original shows separate TS/HTML/SCSS files, do not rewrite them as an inline component. Compress by trimming the original code, not by rewriting it.`;
}
...
async function batchSubmit(opts: CliArgs): Promise<void> {
  ...
  console.log(`\nNext step: npm run compress:angular -- --batch poll`);
}
...
async function processBatchResults(
  ...
  if (failedFiles.length > 0 || invalidFiles.length > 0) {
    console.log(`\nTo retry failed/invalid files: npm run compress:angular -- --batch retry`);
  }
}
...
async function main() {
  ...
  if (!existsSync(INPUT_DIR)) {
    console.error(`Error: Input directory not found: ${INPUT_DIR}`);
    console.error("Run 'npm run inject:angular' first.");
    process.exit(1);
  }
