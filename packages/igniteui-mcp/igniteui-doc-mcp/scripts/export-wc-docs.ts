...
const BUILT_DOCS = join(XPLAT_ROOT, "dist", "WebComponents", "en", "components");
const OUTPUT_DIR = join(ROOT, "dist", "docs_processing", "webcomponents");
...
function main() {
  ...
  if (!skipBuild) {
    console.error("Running xplat gulp build for WebComponents...");
    try {
      execSync("npm run buildWC", {
        cwd: XPLAT_ROOT,
        stdio: "inherit",
      });
    } catch (e) {
      console.error("Failed to run buildWC. Make sure dependencies are installed in common/igniteui-xplat-docs/");
      process.exit(1);
    }
  }
  ...
  if (!existsSync(BUILT_DOCS)) {
    ...
    console.error("Run 'npm run buildWC' inside common/igniteui-xplat-docs/ first.");
    process.exit(1);
  }

  console.error("Parsing toc.json...");
  const tocEntries = parseTocJson(TOC_PATH);
  console.error(`  Found ${tocEntries.length} WebComponents entries in toc.json`);
