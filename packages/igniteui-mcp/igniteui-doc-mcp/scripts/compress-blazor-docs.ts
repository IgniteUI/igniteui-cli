...
const INPUT_DIR = join(ROOT, "dist", "docs_prepeared", "blazor");
const OUTPUT_DIR = join(ROOT, "dist", "docs_final", "blazor");
...
function getSystemPrompt(): string {
  return `You are compressing Infragistics Ignite UI for Blazor component documentation. Your goal is to reduce file size by ~50% while keeping the doc useful for developers building with these components.

FRONTMATTER (REQUIRED):
Start every output with a YAML frontmatter block. This is mandatory and must be the very first thing in the output:

---
component: IgbExactComponentName
...
- **component**: The Ignite UI for Blazor component class name(s) documented here (e.g. IgbGrid, IgbButton, IgbCombo, IgbDatePicker). Use the PascalCase Igb-prefixed name as used in Blazor. Blazor components do NOT use suffixes like Component or Directive — just use the base name (e.g. IgbGrid, not IgbGridComponent). Comma-separate multiple names. These rules are strict:
  - Every name MUST begin with \`Igb\`. A class in the sample code that does not start with \`Igb\` is application code, not a library component.
  - NEVER list classes the sample application defines for itself — page models, \`*Sample\`, demo services, or local record/DTO types. List only components from the Ignite UI library, even when the sample's own classes are more prominent.
...
- **Never change code patterns or structure.** If the original uses a property with a backing field, do not replace it with an auto-property. Compress by removing entire methods/blocks, not by rewriting the ones you keep.
- **Always mark truncation with a comment.** Whenever you remove items from an array, list, Razor children, or repeated block, insert a \`// ...\` or \`@* ... *@\` comment at the removal point. Use \`@* ... *@\` in Razor markup blocks and \`// ...\` in C# code blocks.
- **Code blocks must be self-consistent.** If a code block references a variable, function, data model field, or parameter, that reference must either be defined within the same block or in an adjacent block. Do not remove a definition while keeping code that uses it.
- **Do not strip component parameters from Razor markup.** If the original has \`<IgbDataToolTipLayer Name="tooltips" />\`, keep the \`Name\` parameter. Only remove parameters that are purely cosmetic (inline style for demo layout like \`style="height: 600px"\`).
- **Use correct comment syntax.** Inside Razor markup use \`@* comment *@\`, inside C# code use \`// comment\`. Never use HTML comments (\`<!-- -->\`) inside Razor components — prefer Razor comment syntax.
...
- Do NOT invent or generate new content. Never fabricate code examples, parameter values, API details, or explanations that are not present in the source. If the input document contains only headers and TODO/placeholder comments with no actual documentation, return only the frontmatter and the existing headers.
...
async function batchSubmit(opts: CliArgs): Promise<void> {
  ...
  console.log(`\nNext step: npm run compress:blazor -- --batch poll`);
}
...
async function processBatchResults(
  ...
  if (failedFiles.length > 0 || invalidFiles.length > 0) {
    console.log(`\nTo retry failed/invalid files: npm run compress:blazor -- --batch retry`);
  }
}
...
async function main() {
  ...
  if (!existsSync(INPUT_DIR)) {
    console.error(`Error: Input directory not found: ${INPUT_DIR}`);
    console.error("Run 'npm run inject:blazor' first.");
    process.exit(1);
  }
